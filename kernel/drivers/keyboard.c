...
static void kbd_push(char c) {
  ...
  if (next == kbd_tail)
    return; /* buffer full - drop the character rather than overwrite unread
               ones */
...
static void keyboard_irq_handler(registers_t *regs) {
  ...
  char c = shift_held ? scancode_to_ascii_shifted[scancode]
                      : scancode_to_ascii[scancode];
  if (c == 0)
    return; /* unmapped key (function keys, arrows, ...) - ignore for now */

  kbd_push(c);
  putchar_at_cursor(c); /* immediate visible feedback on screen */
